...
with tf.variable_scope('Input'):
    tf_x = tf.placeholder(tf.float32, [None, 28*28],name='x') / 255.
...
tf.summary.histogram('Pool_Layer1', pool2)
...
for step in range(1000):
    ...
    _, result = sess.run([train_op, merge_op], {tf_x: b_x, tf_y: b_y})
    writer.add_summary(result, step)

    if step % 50 == 0:
        accuracy_, flat_representation = sess.run([accuracy,flat ], {tf_x: test_x, tf_y: test_y})
        print('Step:', step , '| test accuracy: %.2f' % accuracy_)
#         if HAS_SK:
